...
fn main() {
    ...
    layer.use_text(payable_date, 10, Mm(180.0), Mm(259.0), &regular);
    layer.use_text(payable_to, 11, Mm(27.0), Mm(246.0), &regular);
    layer.use_text(padded_amount, 10, Mm(179.0), Mm(246.0), &regular);
    layer.use_text(padded_english, 10, Mm(9.8), Mm(237.2), &regular);

    // mailingaddress
    layer.use_text(payable_to, 11, Mm(22.0), Mm(229.0), &regular);
    layer.use_text(payable_address1, 11, Mm(22.0), Mm(225.0), &regular);
    layer.use_text(payable_address2, 11, Mm(22.0), Mm(221.0), &regular);

    for y in vec![87.2, 180.7].iter() {
        layer.use_text(payable_date, 10, Mm(18.0), Mm(*y), &bold);
        layer.use_text(payable_to, 11, Mm(50.0), Mm(*y), &bold);
    }

    for y in vec![14.0, 105.0].iter() {
        layer.use_text("Checking", 11, Mm(9.1), Mm(*y), &bold);
        layer.use_text(account_last4, 11, Mm(27.0), Mm(*y), &bold);
        layer.use_text("(", 11, Mm(37.0), Mm(*y), &bold);
        layer.use_text(account_last4, 11, Mm(39.0), Mm(*y), &bold);
    ...
    for y in y_starts.iter() {
        for (i, bill_date) in bill_dates.iter().enumerate() {
            ...
            layer.use_text(bill_references[i], 11, Mm(62.0), Mm(*y - offset), &regular);
